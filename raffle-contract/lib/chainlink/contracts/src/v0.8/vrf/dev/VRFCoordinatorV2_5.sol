...
contract VRFCoordinatorV2_5 is VRF, SubscriptionAPI, IVRFCoordinatorV2Plus {
    ...
    mapping(bytes32 => ProvingKey) /* keyHash */ /* provingKey */
    ...
    function deregisterProvingKey(
        ...
        for (uint256 i = 0; i < s_provingKeyHashesLength; ++i) {
            if (s_provingKeyHashes[i] == kh) {
                ...
                s_provingKeyHashes[i] = s_provingKeyHashes[
                    s_provingKeyHashesLength - 1
                ];
    ...
    function requestRandomWords(
        ...
        mapping(uint256 => ConsumerConfig)
            storage consumerConfigs = s_consumers[msg.sender];
        ...
        uint256 preSeed;
        (requestId, preSeed) = _computeRequestId(
            req.keyHash,
            msg.sender,
            subId,
            consumerConfig.nonce
    ...
    function fulfillRandomWords(
        ...
        uint256 gasPrice = _getValidatedGasPrice(
            onlyPremium,
            output.provingKey.maxGas
        );
        ...
        // stack too deep error
        {
            ...
            bool isFeedStale;
            (payment, isFeedStale) = _calculatePaymentAmount(
                startGas,
                gasPrice,
                nativePayment,
                onlyPremium
    ...
    function _calculatePaymentAmount(
        ...
        return
            _calculatePaymentAmountLink(startGas, weiPerUnitGas, onlyPremium);
    }
    ...
    function _calculatePaymentAmountNative(
        ...
        uint256 baseFeeWei = weiPerUnitGas *
            (s_config.gasAfterPaymentCalculation + startGas - gasleft());
        // calculate flat fee in native
        uint256 flatFeeWei = 1e12 *
            uint256(s_config.fulfillmentFlatFeeNativePPM);
        ...
        if (onlyPremium) {
            return
                uint96(
                    (((l1CostWei + baseFeeWei) *
                        (s_config.nativePremiumPercentage)) / 100) + flatFeeWei
                );
        } else {
            return
                uint96(
                    (((l1CostWei + baseFeeWei) *
                        (100 + s_config.nativePremiumPercentage)) / 100) +
                        flatFeeWei
                );
        }
    }
    ...
    function _calculatePaymentAmountLink(
        ...
        uint256 flatFeeWei = 1e12 *
            uint256(
                s_config.fulfillmentFlatFeeNativePPM -
                    s_config.fulfillmentFlatFeeLinkDiscountPPM
            );
        ...
        uint256 payment;
        if (onlyPremium) {
            payment = ((paymentNoFee * (s_config.linkPremiumPercentage)) /
                100 +
                flatFeeJuels);
        } else {
            payment = ((paymentNoFee * (100 + s_config.linkPremiumPercentage)) /
                100 +
                flatFeeJuels);
        }
        if (payment > 1e27) {
    ...
    function removeConsumer(
        ...
        address[] storage s_subscriptionConsumers = s_subscriptionConfigs[subId]
            .consumers;
        uint256 consumersLength = s_subscriptionConsumers.length;
        for (uint256 i = 0; i < consumersLength; ++i) {
            if (s_subscriptionConsumers[i] == consumer) {
                // Storage write to preserve last element
                s_subscriptionConsumers[i] = s_subscriptionConsumers[
                    consumersLength - 1
                ];
    ...
    function migrate(
        ...
        IVRFCoordinatorV2PlusMigration(newCoordinator).onMigration{
            value: nativeBalance
        }(encodedData);
        ...
        if (address(LINK) != address(0) && balance != 0) {
            _requireSufficientBalance(
                LINK.transfer(address(newCoordinator), balance)
            );
        }
        ...
        uint256 consumersLength = consumers.length;
        for (uint256 i = 0; i < consumersLength; ++i) {
            IVRFMigratableConsumerV2Plus(consumers[i]).setCoordinator(
                newCoordinator
            );
        }
        s_config.reentrancyLock = false;
