...
abstract contract BaseTest is Test {
    ...
    function _findVRFRequestIdFromCoordinatorLogs(Vm.Log[] memory logs, address coordinator)
        internal
        pure
        returns (uint256)
    {
        ...
        for (uint256 i = logs.length; i > 0; i--) {
            ...
            (uint256 requestId,,,,,) = abi.decode(log.data, (uint256, uint256, uint16, uint32, uint32, bytes));
            return requestId;
        }
        return 0;
    }
